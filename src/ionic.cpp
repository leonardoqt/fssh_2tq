...
void ionic::try_hop()
{
	...
	H->adiab_parallel(x_t,V0,E0,V_tmp,U_tmp,T_tmp);
	V0 = V_tmp;
	double E_pot1 = E0(istate), E_k1 = 0.5*mass*dot(v_t,v_t);
	// velocity rescale of VV
	if (E_pot0+E_k0-E_pot1>0)
		v_t = sqrt((E_pot0+E_k0-E_pot1)/E_k1)*v_t;
	else
		v_t = 0*v_t;
	//
	if ( jstate != istate)
